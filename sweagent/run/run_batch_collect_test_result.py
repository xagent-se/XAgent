...
class RunBatch:
    ...
    def run_instance(self, instance: BatchInstance) -> None:
        ...
        #     # check if the trace.json and trace_non_bug.json exist
        #     # output_dir = Path(self.output_dir) / instance.problem_statement.id
        #     # output_dir.mkdir(parents=True, exist_ok=True)
        #     # trace_path = output_dir / f"{instance.problem_statement.id}.trace.json"
        #     # trace_non_bug_path = output_dir / f"{instance.problem_statement.id}.trace_non_bug.json"
        #     # # if not trace_path.exists() and not trace_non_bug_path.exists():
        #     result = self._run_instance(instance, run_agent=False)
    ...
    def _run_instance(self, instance: BatchInstance, out_dir: Path, run_agent: bool = True) -> AgentRunResult:
        ...
        if unit_tests_folder.exists():
            for file in unit_tests_folder.glob("test*.py"):
                ...
                unit_tests_files.append(f"unit_tests/{file.name}")
                
        # copy all variant patches to the environment
        variant_patches_json = Path("all_variant_patches.json")
        variant_patches = json.load(open(variant_patches_json))
        patches = variant_patches[instance.problem_statement.id]
        patch_files = []
        for i, patch in enumerate(patches):
            env.write_file(f"variant_patches/{instance.problem_statement.id}_{i}.diff", patch)
            print(f"Copied {patch} to {env.read_file(f'variant_patches/{instance.problem_statement.id}_{i}.diff')}")
            patch_files.append(f"variant_patches/{instance.problem_statement.id}_{i}.diff")
        # commit the current state
        env.communicate("git add .", check="ignore")
        env.communicate("git commit -m 'apply variant patches'", check="ignore")
            
        # get the current commit hash
        commit_hash = env.communicate("git rev-parse HEAD", check="ignore").strip()
        print(f"Current commit hash: {commit_hash}")
        
        # for every patch, apply the patch to the current commit, then all tests for each patch, save the results of the tests to a variable.
        test_results = {i: None for i in range(len(patches))}
        for i, patch_file in enumerate(patch_files):
            env.communicate(f"git apply {patch_file}", check="ignore")
            test_results[i] = {}
            test_files = unit_tests_files
            for test_file in test_files:
                test_results[i][test_file] = env.communicate(f"python {test_file}", check="ignore").strip()
                print(f"Test results for {instance.problem_statement.id}_{i}: {test_results[i][test_file]}")
                
            # force reset the commit
            env.communicate(f"git reset --hard {commit_hash}", check="ignore")
            print(f"Reverted to commit hash: {commit_hash}")
            
        test_output_dir = "test_results"
        if not Path(test_output_dir).exists():
            Path(test_output_dir).mkdir(parents=True, exist_ok=True)
        
        # save the test_results to a json file
        json.dump(test_results, open(f"test_results/{instance.problem_statement.id}_test_results.json", "w"), indent=2)
        
        # pretty print to txt file
        with open(f"test_results/{instance.problem_statement.id}_test_results.txt", "w") as f:
            for i, test_result in test_results.items():
                f.write(f"========== Patch {i} ==========\n")
                for test_file, test_result in test_result.items():
                    f.write(f"{test_file}:\n{test_result}\n")
                    f.write("\n")
