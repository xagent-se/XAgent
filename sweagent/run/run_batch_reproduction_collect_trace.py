...
class RunBatch:
    ...
    def run_instance(self, instance: BatchInstance) -> None:
        ...
        # over the instances
        try:
            result = self._run_instance(instance, run_agent=True)
    ...
    def _run_instance(self, instance: BatchInstance, run_agent: bool = True) -> AgentRunResult:
        output_dir = Path(self.output_dir) / instance.problem_statement.id
        output_dir.mkdir(parents=True, exist_ok=True)
        
        env = self._setup_env(instance, output_dir)
        # return AgentRunResult(
        #     info={"exit_status": "unknown_exit"},
        #     trajectory=[],
        # )
        if run_agent:
            try:
                result = self._run_agent(instance, env, output_dir)
                # self.docker_command(result, env, output_dir, instance)
            finally:
                env.close()
        else:
