...
class RunBatch:
    ...
    def _run_instance(self, instance: BatchInstance, out_dir: Path, run_agent: bool = True) -> AgentRunResult:
        ...
        # copy {reproduction_traj_dir}/{instance_id}
        # to the output_dir
        if not output_dir.exists():
            self.logger.info(f"Copying trajectory from {reproduction_traj_dir}/{instance.problem_statement.id} to {output_dir}")
            # only copy the .traj file
            traj_path = Path(reproduction_traj_dir) / instance.problem_statement.id / f"{instance.problem_statement.id}.traj"
            if traj_path.exists():
                shutil.copy(
                    Path(reproduction_traj_dir) / instance.problem_statement.id / f"{instance.problem_statement.id}.traj",
                    output_dir / f"{instance.problem_statement.id}.traj"
                )
            # shutil.copytree(
            #     Path(reproduction_traj_dir) / instance.problem_statement.id,
            #     output_dir
            # )
        else:
