    ...
    def handle_action(self, step: StepOutput) -> StepOutput:
        ...
        try:
            step.observation = self._env.communicate(
                input=make_no_pager_config(run_action),
...
def make_no_pager_config(cmd: str) -> str:
    """Make the no pager config for the commands."""
    # if the "cmd" contains "git log" or "git diff" or "git show", then replace by "git --no-pager log" or "git --no-pager diff" or "git --no-pager show"
    if "git log" in cmd and "git --no-pager" not in cmd:
        return cmd.replace("git log", "git --no-pager log")
    elif "git diff" in cmd and "git --no-pager" not in cmd:
        return cmd.replace("git diff", "git --no-pager diff")
    elif "git show" in cmd and "git --no-pager" not in cmd:
        return cmd.replace("git show", "git --no-pager show")
    return cmd